...
latency, tokens, and cost-per-query head-to-head.
...
# Naive baseline replay — "Convert this question to SQL: {q}", no schema, no repair
...
# Fireworks — latency + cost at the projected reference volume
...
- `FIREWORKS_API_KEY` — required only for the Fireworks-side scripts
  ...
  Live Demo when toggled to Fireworks.
...
Qwen2.5-Coder-32B-Instruct was the original candidate, but it was
not reachable from the Fireworks key used. Kimi K2.6 was selected
...
| Qwen2.5-Coder-32B | — | — | — | — | Original candidate · not reachable from the key used. |
...
### Naive baseline (`evals.py --baseline`)

A naive prototype prompt (`Convert this question to SQL:
...
| Kimi K2.6 + naive baseline prompt | 2/4 | 0/4 | 0/2 | 2/10 (20%) |
...
The benchmark's latency target is sub-3 s P50 end-to-end. The
...
The 30k q/day volume models a reference deployment (1,000 users × 30
queries/day).
GPT-5.4 baseline rates are the published $2.50 input / $15.00 output
...
loads sql.js + Chinook.db in the browser and lets the reader:

- Re-run any of the 25 questions live against Fireworks (their own key)
  and see SQL, results, pass/fail vs gold answers.
- Run the naive baseline prompt and the agent prompt side-by-side
...
1. **Sub-3 s P50 on shared serverless is not contractual.** Across 30
   ...
   deployment — see the roadmap. A 3 s P50 should not be treated as
   guaranteed on shared serverless.
...
3. **Schema trim is keyword-based, not retrieval.** Fires on 9 of 25
   ...
   For a 1,000-table production DB this isn't enough; we'd need a real
...
2. **Schema retrieval for production.** Replace the full schema dump with
   ...
   makes the architecture survive a 1000-table production database.
3. **CACR-style router** (cheap-first, escalate on low confidence). The
   suite already has the building blocks: Qwen3-8B handles the easy
...
4. **Multi-run perf measurement** with separate model-time and
   wall-time tracking, so reported numbers aren't contaminated by
   429 backoff sleeps.
