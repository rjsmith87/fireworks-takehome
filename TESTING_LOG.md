# Testing Log — Inference Benchmarking Suite
...
- `python -m src.evals --questions data/synthetic_questions.json` →
  **13/15 (87%)** on a clean run; 14/15 on quieter shared-tier runs
  ...
  be conservative.
- `python -m src.evals --baseline` → **2/10**. Confirms the naive
  prototype prompt (`Convert this question to SQL: {question}`)
...
first time a user asks "how many tracks have 'love' in the name".
...
surfaced in front of a real user instead of in eval.
...
- Across 30 perf calls: 23% under 3 s, 33% under 5 s, 53% under 10 s.
  ...
  slow when the deployment is loaded. **The 3-second P50 target
  is achievable on a quiet shared tier with our compact schema
...
- Did not actually run SFT/RFT/DPO. Section 10 of the dashboard and
  ...
  rationale, but no checkpoints were trained. The time budget for
  this pass didn't accommodate a meaningful training run.

### Retrieval-based schema selection
- The keyword-match + FK-closure trim is pragmatic for an 11-table
  schema. For a 1,000-table production database we'd need to embed
...
- I did **not** open it in Chrome/Safari/Firefox myself and click
  through every interactive element with a real Fireworks key. A
  reader can do this — every section has a "test it yourself"
...
### Held-out eval set
- No separate held-out set was used; my 23/25 number is on the
  10 dev + 15 synthetic questions only. The synthetic set covers patterns the dev set
